@echo off
rem Configuration for the full runtime package. It uses the bundled ARTIQ 8
rem environment instead of installing ARTIQ from the current MSYS2 repository.
...
set "TTL_EDGE=rising"
rem TTL voltage threshold is set by the external TTL/discriminator hardware.
rem Software-adjustable counting parameters:
